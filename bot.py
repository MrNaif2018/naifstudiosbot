...
def lock(message):
    ...
    for i in data:
        if i[0] == message.chat.id:
            data_chat = i
    if bot.get_chat_member(message.chat.id,message.from_user.id).status == "administrator" or bot.get_chat_member(message.chat.id,message.from_user.id).status == "creator" or message.chat.type != "supergroup" or message.from_user.id == 524253028:
        global back
...
def test(sent):
    ...
    for i in data:
        if i[0] == sent.message.chat.id:
            data_chat=i
    if bot.get_chat_member(sent.message.chat.id,sent.from_user.id).status == "administrator" or bot.get_chat_member(sent.message.chat.id,sent.from_user.id).status == "creator" or sent.message.chat.type != "supergroup" or sent.from_user.id == 524253028:
...
def top(message):
    ...
    for i in data:
        if i[0] == message.chat.id:
            data_chat=i
    if ((data_chat[6] != 1) or (bot.get_chat_member(message.chat.id,message.from_user.id).status == "administrator" or bot.get_chat_member(message.chat.id,message.from_user.id).status == "creator")) and data_chat[6] != 2:
        tmp={}
        for i in users:
            if i[0] == message.chat.id:
                tmp[i[1]]=i[2]
        ...
        iii=1
        for i in range(0,len(top)):
            if bot.get_chat_member(message.chat.id,top[i][0]).user.username != None:
                a+=str(iii)+". "+str(bot.get_chat_member(message.chat.id,int(top[i][0])).user.username)+": "+str(top[i][1])+"\n"
            else:
                a+=str(iii)+". "+str(bot.get_chat_member(message.chat.id,int(top[i][0])).user.first_name)+": "+str(top[i][1])+"\n"
            iii+=1
        if top != {}:
...
def rate_check(message):
    ...
    if ((data_chat[6] != 1) or (bot.get_chat_member(message.chat.id,message.from_user.id).status == "administrator" or bot.get_chat_member(message.chat.id,message.from_user.id).status == "creator")) and data_chat[6] != 2:
        ...
        if message.reply_to_message != None:
            ...
            if message.from_user.username == message.reply_to_message.from_user.username and ("+" in message.text or "-" in message.text):
                bot.send_message(message.chat.id,"Нельзя оценивать самого себя!",reply_to_message_id=message.message_id)
    text=message.text
    for i in mats:
        if i in text:
            bot.delete_message(message.chat.id,message.message_id)
            ...
            if warn_n == 3:
                if bot.get_chat_member(message.chat.id,message.from_user.id).status != "administrator" and bot.get_chat_member(message.chat.id,message.from_user.id).status != "creator":
